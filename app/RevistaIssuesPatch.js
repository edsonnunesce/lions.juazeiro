...
(function(){
  ...
  function current(){
    const m=location.pathname.match(/\/revista\/(al-2025-2026-00[123])/);
    if(m) return issues.find(x=>x.id===m[1])||issues[1];
    return issues[1];
  }
  function ensureStyles(){
    if(document.getElementById('revista-issues-style')) return;
    const s=document.createElement('style');
    s.id='revista-issues-style';
    s.textContent='.issueThumb.soon{border-color:#cbd5e1;background:#f8fbff}.issueThumb.activeIssue{outline:3px solid #ffcc00}.soonPage{display:grid;place-items:center;text-align:center}.soonPage h2{font-size:54pt}.issueBadge{display:inline-flex;border-radius:999px;background:#fff8d6;border:1px solid #ffcc00;padding:4px 10px;color:#00338d;font-weight:950;margin-top:8px}';
    document.head.appendChild(s);
  }
  function rewriteSidebar(issue){
    const side=document.querySelector('.sideIssues');
    if(!side)return;
    side.innerHTML=issues.map(x=>'<a class="issueThumb '+(x.status==='em breve'?'soon':'')+' '+(x.id===issue.id?'activeIssue':'')+'" href="/revista/'+x.id+'"><div class="thumbCover"><span>'+(x.status==='em breve'?'Em breve':'Edição disponível')+'</span><b>AL nº '+x.number+'</b><span>'+x.month+' · '+(x.from?('campanhas de '+x.from.slice(5,7)+'/'+x.from.slice(0,4)):'')+'</span></div><div class="thumbMeta"><strong>Revista Mensal AL 2025/2026 número '+x.number+'</strong><small>'+x.month+(x.status==='em breve'?' · em breve':' · ações do mês anterior')+'</small></div></a>').join('');
  }
  function replaceText(root, issue){
    const txt=['Revista Mensal AL 2025/2026 número 1','Revista Mensal AL 2025/2026 numero 1','Revista Mensal AL 2025/2026 número 2'];
    const newTitle='Revista Mensal AL 2025/2026 número '+issue.number;
    document.querySelectorAll('.issueHeader h2,.thumbMeta strong').forEach(el=>{el.textContent=newTitle;});
    ...
    document.querySelectorAll('.coverStrip div:first-child b,.thumbCover b').forEach(el=>{if(el.closest('.activeIssue')||el.closest('.coverStrip')) el.textContent=issue.month;});
    document.querySelectorAll('.coverStrip div:first-child span').forEach(el=>{el.textContent='AL 2025/2026';});
    document.querySelectorAll('.backCover p').forEach(el=>{el.textContent=el.textContent.replace('Edição nº 1','Edição nº '+issue.number);});
  }
  ...
  function fillIssue2Research(){
    ...
    if(p11){const h=p11.querySelector('h2'); if(h) h.textContent='Fontes pesquisadas para julho'; const ol=p11.querySelector('ol'); if(ol){const li=document.createElement('li');li.textContent='Pesquisa editorial de junho/julho: Lions International, LCIF, LION Magazine, Lions Brasil, Distrito Múltiplo LA e Distrito LA-4. Os campos locais seguem alimentados por campanhas cadastradas no banco.';ol.prepend(li);} }
  }
