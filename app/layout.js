...
const globalEnhancements = `
  ...
  .searchModal { width: min(760px, 96vw); max-height: 84vh; overflow: auto; background: #fff; border-radius: 24px; border: 1px solid #d9e2ef; box-shadow: 0 30px 80px rgba(0,25,80,.35); padding: 22px; }
  .searchHead { display: flex; justify-content: space-between; align-items: center; gap: 14px; margin-bottom: 14px; }
  .searchHead h2 { margin: 0; color: #00338d; font-size: 1.55rem; }
  .searchClose { width: 42px !important; height: 42px !important; max-width: 42px !important; min-height: 42px !important; border-radius: 50% !important; padding: 0 !important; background: #eef4ff !important; color: #00338d !important; display: inline-flex !important; align-items: center !important; justify-content: center !important; }
  .searchForm { display: grid; grid-template-columns: 1fr auto; gap: 10px; margin-bottom: 14px; }
  .searchForm input { width: 100%; min-height: 52px; border: 1px solid #d9e2ef; border-radius: 16px; padding: 0 15px; font: inherit; outline: none; }
  .searchForm input:focus { border-color: #00338d; box-shadow: 0 0 0 4px rgba(0,51,141,.10); }
  .searchForm button { min-height: 52px !important; border-radius: 16px !important; max-width: none !important; padding: 0 20px !important; background: #ffcc00 !important; color: #111827 !important; }
  .searchHint { margin: 0 0 14px; color: #65758b; font-size: .92rem; }
  .searchResults { display: grid; gap: 10px; }
  .searchResult { display: block; border: 1px solid #d9e2ef; border-radius: 18px; padding: 15px; background: #fff; }
  ...
  .searchEmpty { padding: 16px; border-radius: 18px; background: #fff8d6; border: 1px solid #fde68a; color: #634900; }
...
(function(){
  ...
  function runSearch(query){
    ...
    if (!q) {
      box.innerHTML = pages.map(function(page){ return '<a class="searchResult" href="' + page.url + '"><b>' + page.title + '</b><span>' + page.text.slice(0,145) + '...</span></a>'; }).join('');
      return;
    }
    ...
    box.innerHTML = results.map(function(page){ return '<a class="searchResult" href="' + page.url + '"><b>' + page.title + '</b><span>' + page.text.slice(0,165) + '...</span></a>'; }).join('');
  }
